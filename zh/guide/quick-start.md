...
首次运行时，Rnix 会自动启动一个后台 daemon 进程来管理内核和进程表。daemon 通过 Unix domain socket 与 CLI 通信，空闲 60 秒后自动退出。你也可以通过 `rnix daemon status` 查看状态，或 `rnix daemon stop` 手动停止。
...
优先级：`--provider` CLI flag > agent.yaml `models.provider` > 默认 `claude`
...
想要手把手学习 Rnix 开发？查看 [教程目录](/zh/tutorials/)：

1. **[编写第一个 Skill](/zh/tutorials/writing-first-skill)** — 创建 Skill、Agent 并运行（~20 分钟）
2. **[调试第一个 bug](/zh/tutorials/debugging-first-bug)** — 使用 strace 定位和修复问题（~15 分钟）
3. **[组合多智能体工作流](/zh/tutorials/composing-multi-agent-workflow)** — 用 Compose 编排多智能体协作（~25 分钟）
