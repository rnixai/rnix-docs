...
- **失败调用同样会被记录** —— 出错的调用会带 `outcome: error` 标记与错误文本一并捕获，在响应段之前渲染为一行 `⚠ outcome: error — <error>`。失败恰恰是最需要取回真实请求的时刻，因此「primary 失败 + fallback 成功」会为同一 step 留下两条记录。
- 同一批捕获也可从 Dashboard inspector 的 **Raw I/O** 视图以及 IPC 获取，三者共用同一后端。

`raw_capture` 配置键见 [配置 › 原始请求记录](/zh/guide/configuration#raw-capture)。

---

## 编排退出码

当一个进程编排子任务时，其退出码反映的是**真实**失败，而非偶发噪声。自 0.11.0 起：

- 成功完成的进程**不再仅因一次被处理的、非致命的工具错误就报告失败的退出码**。一次出错但已被恢复的工具调用，本身不会让父进程失败。
- 失败改由真实信号驱动：真正的子任务失败，或**重复**的工具错误触发累积工具错误熔断器。

这使得非零退出码成为「确实出了问题」的可靠信号，`rnix wait` 与 CI 闸门因此可以信任它。
