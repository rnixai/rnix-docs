...
`rnix wait <pid>` 会阻塞至进程到达终止状态，随后传播其退出码，为外部工具提供一个 Unix `waitpid` 风格的 join 点，可等待**任意**进程——不限于它自己 spawn 的进程。已结束的进程会立即从历史中返回。`--timeout` 为等待设定上限（超时则以退出码 124 退出），`--json` 输出机器可读结果。

完整标志与输出格式见 [CLI 参考](/zh/reference/cli)。

## 将外部进程挂入进程树

从运行中的 Rnix 进程**外部**启动的进程——例如一个 shell 脚本调用 `rnix`——默认在进程树中没有父节点。`--parent <pid>` spawn 选项（也可通过 `RNIX_PARENT_PID` 环境变量设置）会把这样的进程挂到进程树中正确的位置，使 Dashboard 显示准确的父子关系，且 spawn 深度限制依然适用。

它与 `rnix wait` 互补：外层脚本可用 `--parent` spawn 任务，再对返回的 PID 执行 `rnix wait` 同步 join。
