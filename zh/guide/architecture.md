...
本文档面向希望深入理解 Rnix 内部设计的贡献者。阅读前建议先熟悉 [核心概念](/zh/guide/concepts)，这里不再复述概念定义，而是聚焦**设计决策、接口边界、数据流和扩展路径**。

> 如需查询具体 API 签名和参数细节，请参阅 [参考手册](/zh/reference/)。
> 如需实战操作指引，请参阅 [教程](/zh/tutorials/)。
...
| `/dev/llm/cursor` | `drivers/llm` | LLM 调用（Cursor CLI） |
...
| 驱动 | VFS 路径 | CLI 命令 | 特点 |
|------|----------|----------|------|
| `ClaudeCliDriver` | `/dev/llm/claude` | `claude -p` | 支持 `--system-prompt`、`--max-turns` |
| `CursorCliDriver` | `/dev/llm/cursor` | `agent --print` | 系统提示词拼接到 prompt、无 `--max-turns` |

添加新 LLM 驱动只需实现 `LLMDriver` 接口并注册到 VFS。内核 Spawn 通过 `resolveLLMDevice()` 根据 `--provider` CLI flag 或 agent.yaml `models.provider` 选择设备路径，reasonStep 通过 VFS Read/Write 与 LLM 交互，不直接依赖具体驱动实现。
...
- [核心概念](/zh/guide/concepts) — 建立 Rnix 的心智模型
- [参考手册](/zh/reference/) — 精确的 API 签名和参数细节
- [教程](/zh/tutorials/) — 手把手实战操作
  - [编写第一个 Skill](/zh/tutorials/writing-first-skill)
  - [调试第一个 bug](/zh/tutorials/debugging-first-bug)
  - [组合多智能体工作流](/zh/tutorials/composing-multi-agent-workflow)
